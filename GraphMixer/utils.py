...
def row_norm(adj_t):
    if isinstance(adj_t, jittor_sparse.SparseTensor):
        # adj_t = jittor_sparse.fill_diag(adj, 1)
        deg = jittor_sparse.sum(adj_t, dim=1)
        deg_inv = 1. / deg
        deg_inv.masked_fill_(deg_inv == float('inf'), 0.)
        adj_t = jittor_sparse.mul(adj_t, deg_inv.view(-1, 1))
        return adj_t

def sym_norm(adj):
    if isinstance(adj, jittor_sparse.SparseTensor):
